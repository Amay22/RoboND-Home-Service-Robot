...
bool robot_move(const ROBOT_MOVEMENT move_type)
{
    if (move_type == STOP) {
        ...
        ROS_INFO("[ROBOT] MOVE FORWARD \n");
        motor_command.angular.z = 0.0;
        motor_command.linear.x = 0.5;
    } else if (move_type == BACKWARD) {
        ROS_INFO("[ROBOT] MOVE BACKWARD \n");
        ...
        ROS_INFO("[ROBOT] Invalid Move Type \n");
        return false;
    }
...
void laser_callback(const sensor_msgs::LaserScan::ConstPtr& scan_msg)
{
    ...
    for (size_t i = 0; i < range_size; i++) {
        ...
        if (i < range_size / 4 && laser_ranges[i] > range_max) {
...
int main(int argc, char** argv)
{
    ...
    motor_command_publisher = nodeHandle.advertise<geometry_msgs::Twist>("cmd_vel_mux/input/teleop", 100);
